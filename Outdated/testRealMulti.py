...
if __name__ == '__main__':
    ...
    b1 = Block(12, img_size, IFReal(threshold=8 * 2 ** 12),
               SimplifiedSTDP(
                  eta_up=0.3,
                  eta_down=-0.3,
                  mp=True)
    ...
    sim = SimulatorMp(network=model, dataset=image_dataset, dt=0.05, input_period=1, batch_size=200, processes=3)
...
#############################
    ...
    b2 = Block(12, (14, 14), IF(threshold=0.1))
    b2.set_inhibition(wta=True, radius=0)

    b3 = Block(100, (14, 14), IFReal(threshold=12),
               SimplifiedSTDP(
                  eta_up=0.3,
                  eta_down=-0.3)
               )
