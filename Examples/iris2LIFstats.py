...
"""
See iris2LIF, but with several experiments in succession to have an average idea of the performance
"""
if __name__ == '__main__':
    ...
    for exp in range(n_exp):
        ...
        l1 = SimplifiedSTDP(eta_up=0.5, eta_down=-0.5, mp=enableMP)
        e1 = EncoderGFR(
            size=data_size, depth=en1,
            in_min=0, in_max=1,
            threshold=0.9, gamma=0.8, delay_max=1,
            spike_all_last=True
        )
        b1 = Block(
            depth=1, size=n1,
            neuron_type=LIF(threshold=1.9, tau=0.5),
            learner=l1
        )
        c1 = Connection(e1, b1, mu=0.5, sigma=0.20, wmin=0, wmax=1)
        ...
        if enableMP:
            ...
            sim = Simulator(network=network, dataset=train, dt=0.01, input_period=1, batch_size=3)
        ...
        c2 = Connection(b1, b2, wmin=0, mu=0.5, sigma=0.1, real=True)
        ...
        # b2.set_inhibition(wta=False, k_wta_level=1)
