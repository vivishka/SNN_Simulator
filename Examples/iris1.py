...
"""
Classifies iris dataset with all to all network and various features
"""
...
if __name__ == '__main__':
    ...
    sim = Simulator(network=model, dataset=image_dataset, dt=0.01, input_period=1, batch_size=1)
    # sim = SimulatorMp(network=model, dataset=image_dataset, dt=0.01, input_period=1, batch_size=25, processes=3)
    ...
    b1 = Block(depth=1, size=n1, neuron_type=IF(threshold=4),
    ...
    b2 = Block(depth=1, size=n2, neuron_type=IF(threshold=1), learner=Rstdp(
        eta_up=0.8,
        eta_down=-0.8,
        anti_eta_up=-0.2,
        anti_eta_down=0.2,
