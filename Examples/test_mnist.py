...
from classes.layer import Block
...
if __name__ == '__main__':
    ...
    # Layers
    b1 = Block(depth=depth_l1, size=img_size, neuron_type=IF(threshold=10), learner=l1)
    p1 = Block(depth=depth_l1, size=size_l2, neuron_type=PoolingNeuron())

    b2 = Block(depth=depth_l2, size=size_l2, neuron_type=IF(threshold=10), learner=l1)
    p2 = Block(depth=depth_l2, size=size_l3, neuron_type=PoolingNeuron())

    b3 = Block(depth=depth_l3, size=size_l3, neuron_type=IF(threshold=0.01), learner=l2)
    ...
    if enableMP:
        sim = SimulatorMp(network=network, dt=0.05, batch_size=100, input_period=1, dataset=test_set, processes=3)
    else:
